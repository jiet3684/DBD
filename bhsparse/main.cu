...
#define ITER 1
    for (int iter=0; iter<ITER; ++iter){
        ...
        filewrite(csrRowPtrC, csrColIndC, (float*)csrValC, m, nnzC);
        gettimeofday(&ed, NULL);
        printf("%f\t%d\n", (float)(ed.tv_sec - st.tv_sec) + 0.000001 * (float)(ed.tv_usec - st.tv_usec), nnzC);
    ...
    printf("\nAverage\n%f\t%f\t%f\n", t_kernel/ITER, t_d2h/ITER - t_kernel/ITER, t_write/ITER - t_d2h/ITER);
