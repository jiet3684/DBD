...
void MultiplyspECKImplementation(const dCSR<DataType> &matA_Dealloc, const dCSR<DataType> &matB_Dealloc, dCSR<DataType> &matOut, spECKConfig &config, Timings &timings)
{
    ...
    if (d_blockStartRows != nullptr) cudaFree(d_blockStartRows);
        //HANDLE_ERROR(cudaFree(d_blockStartRows));
    if (hashMaps != nullptr)
        HANDLE_ERROR(cudaFree(hashMaps));
    if (maps_indices != nullptr)
...
template void MultiplyspECKImplementation<float, 4, 1024, spECK_DYNAMIC_MEM_PER_BLOCK, spECK_STATIC_MEM_PER_BLOCK>(const dCSR<float> &matA_Dealloc, const dCSR<float> &matB_Dealloc, dCSR<float> &matOut, spECKConfig &config, Timings &timings);
...
void TransferResult(int *ptr, int *idx, float *val, dCSR<float> &matOut, int nr, int ne) {
    cudaMemcpy(ptr, matOut.row_offsets, sizeof(int) * (nr + 1), cudaMemcpyDeviceToHost);
    cudaMemcpy(idx, matOut.col_ids, sizeof(int) * ne, cudaMemcpyDeviceToHost);
    cudaMemcpy(val, matOut.data, sizeof(float) * ne, cudaMemcpyDeviceToHost);
}
